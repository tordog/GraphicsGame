lambertianMultiLightShader = function (gl, nLights, nSpotLights, nAreaLights) {
	...
	for(int iy =  0; iy < 3;  ++iy)\n\
		...
		{\n\
			...
			if(dot(L,n.xyz) > 0.0) {\n\
				...
				lambert +=   (vdiffuse.xyz * uAreaLightsColor[i].xyz) *NdotL;                \n\
			}\n\
		} \n\
 } \n\
\n\
...
lambertianSingleColorMultiLightShader = function (gl, nLights, nSpotLights, nAreaLights) {
...
\n\
  ...
  {																											\n\
	...
	for(int iy =  0; iy < 3;  ++iy)												\n\
		for(int ix =  0; ix < 2;  ++ix)											\n\
		{																										\n\
			float y = float(iy)* (uAreaLightsSize[i].y / 2.0 )\n\
			- uAreaLightsSize[i].y / 2.0;											\n\
			float x = float(ix)* (uAreaLightsSize[i].x / 1.0 )\n\
			- uAreaLightsSize[i].x / 2.0;											\n\
			vec4 lightPos = uAreaLightsFrame[i] * vec4(x,0.0,y,1.0);\n\
			r = length(lightPos.xyz-vpos);										\n\
			L = normalize(lightPos.xyz-vpos);									\n\
			if(dot(L,n.xyz) > 0.0) {													\n\
				NdotL = max(0.0, dot(N, L))/( 0.01*3.14 * 3.14 *r*r);								\n\
				lambert +=   (uColor.xyz * uAreaLightsColor[i].xyz) *NdotL/(3.0*2.0);\n\
			}\n\
		} \n\
 } \n\
\n\
